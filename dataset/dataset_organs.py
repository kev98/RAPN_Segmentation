...
class DSAD_Dataset_Multiclass(BaseDataset):
    """Dresden Surgucal Anatomy Dataset. Multiclass Version
    Read images and corresponding masks, apply augmentation and preprocessing transformations.
...
class DSAD_Dataset_Binary(BaseDataset):
    """Dresden Surgucal Anatomy Dataset. Binary Version
    Read images and corresponding masks, apply augmentation and preprocessing transformations.
...
# Code to try the multiclass dataloader
'''classes = ['abdominal_wall', 'colon', 'liver', 'pancreas', 'small_intestine', 'spleen', 'stomach', 'other']

dataset = DSAD_Dataset_Multiclass(r"/Volumes/TOSHIBA EXT/DSAD_Dataset/multilabel", classes)
...
# Code to try the binary dataloader
'''classes = ['other', 'abdominal_wall']

dataset = DSAD_Dataset_Binary(r"/Volumes/TOSHIBA EXT/DSAD_Dataset/abdominal_wall", classes)
image, mask = dataset.__getitem__(24)
print(image.shape, mask.shape)
for i in range(len(classes)):
    print(i, np.count_nonzero(mask[:, :, i] == 1))'''
